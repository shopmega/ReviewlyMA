-- Comprehensive Test Script for Notifications System in Supabase (WORKING VERSION)
...
-- Test 8: Test common notification queries that the app uses (FIXED VERSION)
DO $$
DECLARE
    query_result INTEGER;
    notifications_list RECORD;
BEGIN
    -- Test the query used by getNotifications function: SELECT * ORDER BY created_at DESC LIMIT 20
    -- Fixed by removing COUNT(*) from the ORDER BY query
    FOR notifications_list IN 
        SELECT id, title, message, is_read, created_at 
        FROM public.notifications 
        ORDER BY created_at DESC 
        LIMIT 20
    LOOP
        EXIT; -- Just test that we can execute the query
    END LOOP;
    
    RAISE NOTICE '✓ Common notification query executes successfully (tested with LIMIT 20)';

    -- Count the total number of notifications separately
    SELECT COUNT(*) 
    FROM public.notifications 
    INTO query_result;
    
    RAISE NOTICE '✓ Total notifications in system: %', query_result;
END $$;
...
-- Test 11: Test that the notification query from getNotifications function works
DO $$
DECLARE
    notification_count INTEGER;
    notification_record RECORD;
BEGIN
    -- Test the exact query pattern used in getNotifications: select *, order by created_at, limit
    ...
    RAISE NOTICE '✓ getNotifications query pattern works (% notifications tested)', LEAST(notification_count, 20);
    
    -- Test a few records to ensure they have the expected structure
    FOR notification_record IN
        ...
        LIMIT 3
    LOOP
        RAISE NOTICE '  Sample notification: ID=%, Title="%", Read=%', 
                     notification_record.id, 
                     LEFT(notification_record.title, 20) || '...',
                     notification_record.is_read;
    END LOOP;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '- getNotifications function query pattern works';
    RAISE NOTICE '========================================';
END $$;