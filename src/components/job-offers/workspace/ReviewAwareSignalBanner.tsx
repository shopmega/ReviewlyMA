  ...
  return (
    <Card className={`rounded-[1.8rem] border shadow-sm ${tone.surface}`}>
      <CardContent className="p-5">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em]">{t('jobOffers.workspace.signals.reviewAware.eyebrow', 'Review-aware interpretation')}</p>
        <h3 className="mt-2 text-xl font-black tracking-tight">{t(workspace.reviewAwareSignal.title)}</h3>
        <p className="mt-2 text-sm leading-7">{t(workspace.reviewAwareSignal.body)}</p>
      </CardContent>
    </Card>
  );
}
