  ...
  return (
    <div className="rounded-[1.3rem] border border-slate-200 bg-slate-50/70 p-4">
      <div className="flex items-center justify-between gap-3">
        <Badge variant="outline">{t(offer.similarity_label)}</Badge>
      ...
      <p className="mt-2 text-sm text-slate-600">{offer.city || t('jobOffers.result.locationUnclear', 'Unknown city')}</p>
      <div className="mt-3 space-y-2 text-sm text-slate-600">
        <p>{offer.salary_min == null && offer.salary_max == null ? t('jobOffers.workspace.presentation.salaryMissing', 'Salary not shown') : t(formatSalary({
          salaryMin: offer.salary_min ?? null,
          salaryMax: offer.salary_max ?? null,
          payPeriod: offer.pay_period,
        }))}</p>
        <p>{t(formatWords(offer.market_position_label))}</p>
        <p>{t('jobOffers.workspace.comparison.similarityScore', 'Similarity score')}: {offer.similarity_score}</p>
      </div>
      {offer.public_href ? (
        <TrackedActionButton
        ...
        >
          <>
            {t('jobOffers.workspace.comparison.openPublicContext', 'Open public context')}
