...
export function ReviewsPageClient({ business }: ReviewsPageClientProps) {
    ...
    const subRatingAverages = useMemo(() => {
        ...
        business.reviews?.forEach(r => {
            if (r.subRatings?.workLifeBalance) { sums.wlb += r.subRatings.workLifeBalance; counts.wlb++; }
            if (r.subRatings?.management) { sums.mgmt += r.subRatings.management; counts.mgmt++; }
            if (r.subRatings?.careerGrowth) { sums.growth += r.subRatings.careerGrowth; counts.growth++; }
            if (r.subRatings?.culture) { sums.culture += r.subRatings.culture; counts.culture++; }
        });
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="hidden sm:block">
                            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-0.5">
                                ...
                                <span>/</span>
                                <Link href="/businesses" className="hover:text-primary transition-colors">Entreprises</Link>
                                <span>/</span>
                                <span className="truncate max-w-[150px]">{business.name}</span>
                            </div>
                            <h1 className="font-bold text-sm uppercase tracking-wider text-foreground">Tous les avis</h1>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <Card className="border-0 shadow-xl shadow-slate-200/50 dark:shadow-none bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden">
                            ...
                            <CardContent className="px-8 pb-8 pt-0 -mt-12">
                                <div className="relative mb-6 inline-block">
                                    ...
                                    {business.is_premium && (
                                        <Badge className="absolute -bottom-2 -right-2 bg-amber-500 text-white border-none font-bold text-[10px] uppercase shadow-lg">PRO</Badge>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center gap-4 text-sm text-muted-foreground font-medium mb-6">
                                    <span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> {business.city}</span>
                                    <span className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" /> {business.category}</span>
                                </div>
                                ...
                                <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-border/50">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Total</p>
                                            <p className="text-xl font-black text-primary">{business.reviews?.length || 0} avis</p>
                                        </div>
                                    </div>
                                    <p className="text-xs font-medium text-muted-foreground leading-relaxed italic">
                                        Basé sur les retours d'expériences vérifiés de nos utilisateurs.
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
                                ...
                                {ratingStats.map((stat) => (
                                    <div key={stat.rating} className="space-y-1.5">
                                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-tighter">
                                            <span className="flex items-center gap-1">{stat.rating} <Star className="h-3 w-3 fill-yellow-400 text-yellow-400" /></span>
                                            <span className="text-muted-foreground">{stat.count} avis</span>
                                        </div>
                        ...
                        <Card className="border-0 shadow-xl shadow-slate-200/50 dark:shadow-none bg-white dark:bg-slate-900 rounded-[2.5rem]">
                            ...
                            <CardContent className="px-8 pb-8">
                                ...
                                <p className="mt-6 text-[10px] font-bold text-muted-foreground uppercase tracking-widest text-center opacity-60">
                    ...
                    <div className="lg:col-span-8">
                        <div className="mb-8 flex flex-col sm:flex-row gap-4">
                            <div className="relative flex-1 group">
                                ...
                                <Input
                                    ...
                                    placeholder="Rechercher un mot-clé (ex: salaire, ambiance)..."
                                    value={searchTerm}
                        ...
                        <div className="space-y-6">
                            {/* We can pass search term to a modified version or just use the current component */}
                            {/* For now, let's reuse the existing SharedReviewsSection but we might need to filter manually if it doesn't support search */}
