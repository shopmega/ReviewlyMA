...
function buildRoleSummaries(rows: DemandRow[]) {
  const byRole = new Map<string, { count: number; cityCounts: Map<string, number> }>();
  ...
  for (const row of rows) {
    ...
    if (!byRole.has(role)) {
      byRole.set(role, { count: 0, cityCounts: new Map<string, number>() });
    }
  ...
  for (const [role, bucket] of byRole.entries()) {
    const topCity =
      [...bucket.cityCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || 'Ville non specifiee';
    summaries.push({
      role,
      count: bucket.count,
      topCity,
      roleSlug: slugify(role),
      citySlug: slugify(topCity),
    });
  }
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: 'Top referral demand roles | Reviewly MA',
    description: 'Role-level referral demand leaderboard with role-to-city drill-down links.',
    alternates: {
      canonical: `${siteUrl}/referral-demand/roles`,
    },
  };
}

export default async function ReferralDemandRolesPage() {
  ...
  const summaries = buildRoleSummaries(rows);
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12 space-y-8">
      <section className="rounded-2xl border border-border bg-card p-6 md:p-8 space-y-4">
        ...
        <h1 className="text-3xl md:text-4xl font-bold font-headline">Top referral demand roles</h1>
        <p className="text-muted-foreground max-w-3xl">
          Role leaderboard generated from active referral demand listings.
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild variant="outline">
            <Link href="/referral-demand/cities">View top cities</Link>
          </Button>
        </div>
      </section>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <CardTitle className="text-lg">Role rankings</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {summaries.length === 0 ? (
            ...
            summaries.map((item) => (
              <div
                key={`${item.roleSlug}-${item.citySlug}`}
