...
export default function GATestingPage() {
    ...
    setTimeout(() => {
      ...
      if (scriptCheck) {
        ...
        gaScripts.forEach((script, index) => {
          addTestResult(`  Script ${index + 1}: ${(script as HTMLScriptElement).src}`);
        });
      } else {
        addTestResult('❌ GA Scripts: No Google Analytics scripts found');
      }

      // Test 5: Event tracking capability
      if (gtagAvailable) {
        try {
  ...
  const getStatusBadge = (passed: boolean) => {
    if (passed) {
      return <Badge variant="default" className="bg-green-500 hover:bg-green-600"><CheckCircle className="w-3 h-3 mr-1" />Passed</Badge>;
    } else {
    ...
    <div className="container mx-auto py-8 space-y-8 max-w-4xl">
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Comprehensive testing suite for GA4 implementation verification
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Overall health check of Google Analytics integration
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
            ...
            {getStatusBadge(Object.values(checks).every(Boolean)) || 
             <Badge variant="secondary"><AlertCircle className="w-3 h-3 mr-1" />Issues Found</Badge>}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Detailed logs from all verification tests
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-muted-foreground">
              <li><strong>Missing Environment Variable:</strong> Add NEXT_PUBLIC_GA_ID=G-XXXXXXXXXX to .env.local</li>
              <li><strong>Wrong GA Property Type:</strong> Must use GA4 (G-XXXXXXXXXX), not Universal Analytics (UA-XXXXXXXXX)</li>
              <li><strong>Domain Not Whitelisted:</strong> Check GA property settings for allowed domains</li>
              <li><strong>Ad Blockers:</strong> Test in incognito/private browsing mode</li>
              <li><strong>Data Processing Delay:</strong> Real-time data appears instantly, standard reports take 24-48 hours</li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
              ...
              <li>Check .env.local file contains NEXT_PUBLIC_GA_ID</li>
