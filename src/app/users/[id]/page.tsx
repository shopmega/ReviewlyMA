...
export default async function UserProfilePage({ params }: PageProps) {
  ...
  const displayName = user.full_name?.trim() || 'Utilisateur';
  ...
  const averageRating =
    reviewCount > 0
      ? (reviews.reduce((acc: number, review: any) => acc + (review.rating || 0), 0) / reviewCount).toFixed(1)
      : null;
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="flex justify-center md:justify-start gap-6 mt-6">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground uppercase tracking-wide">J'aime recus</div>
              </div>
              {averageRating && (
                <div className="text-center">
                  ...
                  <div className="text-xs text-muted-foreground uppercase tracking-wide">Note moyenne</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4">
          {reviews.length > 0 ? (
            reviews.map((review: any) => (
              <Card key={review.id} className="overflow-hidden hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  ...
                  <p className="text-gray-700 leading-relaxed max-w-none prose-sm">{review.content}</p>

                  <div className="mt-4 pt-4 border-t flex justify-between items-center text-sm text-muted-foreground">
                    <span>Publie le {format(new Date(review.created_at), 'd MMMM yyyy', { locale: fr })}</span>
                  </div>
                </CardContent>
              </Card>
            ))
            ...
            <div className="text-center py-12 bg-white rounded-xl border border-dashed">
              ...
              <h3 className="font-semibold text-lg">Aucun avis publie</h3>
              <p className="text-muted-foreground">Cet utilisateur n'a pas encore partage d'avis.</p>
              <Button asChild className="mt-6 rounded-full">
                <Link href="/">Explorer les entreprises</Link>
              </Button>
            </div>
          )}
        </div>
