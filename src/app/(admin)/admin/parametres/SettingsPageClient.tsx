...
export default function SettingsPageClient({ initialSettings }: { initialSettings: SiteSettings }) {
    const [settings, setSettings] = useState<SiteSettings>(initialSettings);
    const loadFailed = false;
    const [saving, setSaving] = useState(false);
    const [salaryRolesText, setSalaryRolesText] = useState((initialSettings.salary_roles || []).join("\n"));
    const [salaryDepartmentsText, setSalaryDepartmentsText] = useState((initialSettings.salary_departments || []).join("\n"));
    const [salaryIntervalsText, setSalaryIntervalsText] = useState(JSON.stringify(initialSettings.salary_intervals || [], null, 2));
                ...
                <aside className="shrink-0 lg:w-72">
                    ...
                    <div className="mt-8 rounded-[2rem] border border-indigo-500/10 bg-indigo-500/5 p-6">
                        ...
                        <div className="space-y-2 text-[10px] font-bold">
                            ...
                            <div className="flex justify-between"><span className="text-muted-foreground uppercase opacity-60">{t("adminSettings.status.pricing", "Tarification")}</span><span className="text-amber-500">{t("adminSettings.status.configured", "CONFIGUREE")}</span></div>
                        </div>
                    </div>
                </aside>
