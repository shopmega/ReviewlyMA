...
import { getAdminAnalytics, type AdminAnalyticsRange } from '@/app/actions/analytics';
import {
...
export default function AnalyticsPageClient() {
  ...
  const [timeRange, setTimeRange] = useState<AdminAnalyticsRange>('30d');
  ...
  const fetchAnalyticsData = async () => {
    setLoading(true);
    try {
      const realData = await getAdminAnalytics(timeRange);
      if (realData) {
