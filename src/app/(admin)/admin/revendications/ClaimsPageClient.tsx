...
import { getAdminClaimsPageData, type AdminClaimListItem } from "@/app/actions/claim-admin-data";
...
export default function ClaimsPageClient() {
  ...
  const [tempProofMethods, setTempProofMethods] = useState<string[]>(defaultVerificationMethods);
  ...
  async function loadClaimsPageData() {
    setLoading(true);
    try {
      const data = await getAdminClaimsPageData(filterStatus);
      setClaims(data.claims);
      setActiveProofMethods(data.verificationMethods);
      setTempProofMethods((prev) => {
        if (showSettingsModal) {
          return prev;
        }
        return data.verificationMethods;
      });
    } catch (error) {
      console.error('Error fetching claims page data:', error);
      toast({
        title: 'Erreur',
        description: "Impossible de charger les revendications d'administration.",
        variant: 'destructive',
      });
      setClaims([]);
    }
      ...
      <Card className="border-border/40 bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b border-border/20 py-6">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="icon" className="rounded-xl border-border/40 hover:bg-primary/10 transition-colors" onClick={loadClaimsPageData}>
